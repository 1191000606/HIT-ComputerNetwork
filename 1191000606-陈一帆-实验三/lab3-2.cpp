...
void stud_route_add(stud_route_msg *proute)
{
	...
	newTableItem.mask = (1<<31)>>(ntohl(proute->masklen)-1); //知道掩码长度后，网络号置一，主机号置零
...
int stud_fwd_deal(char *pBuffer, int length)
{
	...
	int bestMatch = 0;
	// 判断掩码是否匹配
	for(int i = 0; i < m_table.size(); i ++)//每个路由项逐个去匹配
	{
		if(m_table[i].masklen > longestMatchLen && m_table[i].destIP == (DestIP & m_table[i].mask)) //最大前缀匹配,看哪个路由项与目的IP地址匹配的前缀最长
		{
			bestMatch = i;
			Match = true;

...
int stud_ip_recv(char *pBuffer,unsigned short length)
{
	...
	//校验和错应该最后检验错误
	unsigned short sum = 0; //short也是两个字节
	unsigned short tempNum = 0; 
	for (int i = 0; i < headLength * 2; i++){
		tempNum = ((unsigned char)pBuffer[i*2]<<8) + (unsigned char)pBuffer[i*2 + 1];//因为是每16位(两个字节)作为一个整数来求和
		if (0xffff - sum < tempNum)//校验和也只有两个字节
			sum = sum + tempNum + 1;
		else
			sum = sum + tempNum;
	}
				   ...
				   unsigned int dstAddr,byte protocol,byte ttl)
{
	...
	IPBuffer[0] = 0x45;	//版本号+头长度，版本号为4，长度为5个单位，每个单位4个字节，即头部一共20字节

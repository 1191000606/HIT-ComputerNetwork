...
void ackHandler(char c)
{
    ...
    else if (currentSeq < currentAck && (index >= currentAck || index <= currentSeq))
    {
        ...
        if (index <= currentSeq)
        {
            ...
            ackNums += (SEQ_SIZE - currentAck + index + 1);
            currentAck = index + 1;
        }
    }
}
            ...
            while (true)
            {
                ...
                if (!strcmp(command, "begin"))
                {
                    ...
                    while (true)
                    {
                        ...
                        ack[0] += 1; //因为传0会出问题的，到时候收的时候要减一
                        ack[1] = '\0';

                        int isloss = lossInLossRatio(lossRadio);
                        if (isloss)
                        {
                            printf("Ack with seq %d loss\n", ack[0] - 1);
                        }
                        else
                        {
        ...
        {   
            ...
            while (true)
            {
                ...
                else if (!strcmp(requestBuffer, "hello"))
                {
                    ...
                    int flag = 0;                //第一个包最好不要丢,丢了的话接收端exceptedSeq=0,此时一个ack包都没有，返回ack多少呢
                    srand((unsigned)time(NULL)); //判断丢不丢的时候用
                    while (true)
                    {
                        if (seqIsAvailable() && totalIndex < PACKET_SIZE)
                        {
                            ...
                            printf("send a packet with seq %d\n", currentSeq);
